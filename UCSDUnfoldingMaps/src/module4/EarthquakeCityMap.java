...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		HashMap<String, Integer> countsByCountry = new HashMap<>();
		for (Marker marker: quakeMarkers) {
			if (marker instanceof LandQuakeMarker) {
				String country = (String) marker.getProperty("country");
				Integer count = countsByCountry.getOrDefault(country, 0);
				countsByCountry.put(country, count + 1);
			} else {
				Integer count = countsByCountry.getOrDefault("Ocean", 0);
				countsByCountry.put("Ocean", count + 1);
			}
		}
		println(countsByCountry);
